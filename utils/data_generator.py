...
def get_train_data(from_path, to_path):
    ...
    np.save(os.path.join(to_path, 'train-21-15.npy'), train[:,:,:7])
    np.save(os.path.join(to_path, 'val-21-15.npy'), valid[:,:,:7])
    np.save(os.path.join(to_path, 'test-21-15.npy'), test[:,:,:7])
...
if __name__ == '__main__':
    # xlsx_to_csv_pd()
    get_train_data(from_path='../data/data.csv', to_path='../data/21-15/')
